...
c
 ...
 1    lc = i+3
     ...
     +                              sigth, sigph, XX0
      bytes = bytes + 9*4
c     write(48,*) 'readHAFTmatrix 1 ',sigpA,sigpB,sigth,sigph
...
c
c FD
C       open(unit=runit, file=fname, form='unformatted', access='direct',
C      +     recl=1, err=99)
C       bytes=1
C       write(6,*) ' '
C       read(runit,rec=bytes,err=100) comment
C       write(6,'(a80)') comment
C       write(6,*) '--------------------------------------'
C       bytes = bytes + 80
C       read(runit,rec=bytes,err=100) sigpA(1), sigpA(2), sigpA(3),
C      +                              sigpB(1), sigpB(2), sigpB(3),
C      +                              sigth, sigph, XX0
C       bytes = bytes + 9*4
c      write(48,*) 'readHAFTmatrix 2 ',sigpA,sigpB,sigth,sigph
c      close(runit)
c FD
...
c
      ...
      mass2 = mom(0)**2 - ptot2               ! particle mass does not change
...
c
      ...
      mom(0) = sqrt(mom(1)**2 + mom(2)**2 + mom(3)**2 + mass2)  ! total energy
c
      return
      end
c
...
c
c
chw      subroutine smearHadesPair(pair,mode)
      subroutine smearHadesPair(m,pt,rap,mode)
c
...
c
      include 'readHAFT.inc'
c
chw   real*4 m, pt, rap, sigpt, sigm, sigrap
      real*4 sigpt, sigm, sigrap
      real*4 sampleGauss
      integer*4 retcode, readHAFTmatrix
c
      if (readflag.eq.0) then
...
c
      implicit none
      include 'readHAFT.inc'
      data fname /'buuinput/HadesAcceptanceFilter.acc'/
      data fname2 /'buuinput/HadesPairAcceptanceFilter.acc'/
      data iseed /123/
      data readflag /0/
      data readflag2 /0/
      end
