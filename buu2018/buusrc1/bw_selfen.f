************************************************************************
      function bwdist_selfen(s,idm,ires,m_meson_4,ratio)
************************************************************************
...
*          idm  :  kind of decaying meson = 3,5                        *
...
c      write(*,*)'bwstart',m_meson,ires,brnom
      if(idec.eq.0) then
        br1pi = 1.0 - breta
        brdpi = 0.0
        brnrp = 0.0
...
*           write(*,*)"warning from bwdist: omega decay not possible"
     ...
     +            /(4.0*resm0**2)
            ...
            if(igam .le. 3) then
*             use standard-momentum dep. parametrization for gamma
              q2  =(resm0 - rmass - m_meson)**2 + gam0**2/4.0
              gamnom = gam0*brnom*(kpi/kr)**(2.0*ang+1.0)*
     +                    ((kr2+q2)/(kpi2+q2))**(ang+1.0)
c              write(*,*)  ' bw-self  gamnom ',ires,m_meson,resm0,kpi,kr,
c     &           ang,q2,gam0,
c     &           brnom,gamnom
            else if(igam .eq. 4) then
*             the Manley-parametrization
